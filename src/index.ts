...
const SQLiteStore = SQLiteStoreFactory(session) as unknown as { new (options?: any): session.Store }
app.use(session({
...
app.get('/auth/login', (req, res) => res.render('login', { message: null, title: 'Login' }))

app.get('/auth/signup', (req, res) => res.render('signup', { message: null, title: 'Sign Up' }))
...
app.post('/upload', isAuthenticated, upload.single('file'), async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'File uploaded and text saved successfully.', fileId: result.fileId })
  }
  catch (error: any) {
    console.error(error)
...
app.post('/compare/:id', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    console.error(error)
    res.status(500).json({ error: error.message || 'Error processing document comparison' })
  }
})
...
app.post('/admin/credit-requests/:id', isAuthenticated, isAdmin, async (req, res) => {
  ...
  try {
    ...
    console.error(error)
    res.status(500).json({ error: error.message || `Error ${action === 'approve' ? 'approving' : 'denying'} request` })
  }
})
